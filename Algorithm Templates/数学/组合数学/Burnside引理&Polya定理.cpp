...
*/
/*
旋转：顺时针旋转i格的置换中，循环的个数为gcd(n,i)，每个循环的长度为n/gcd(n,i)。
翻转：
当n为奇数时，n/2个循环长度为2，还有一个循环长度为1
当n为偶数时，有n/2个置换满足有n/2个循环长度为2，有n/2个置换有n/2-1个循环长度为2，两个循环的长度为1。
优化：计算旋转时不枚举i枚举L。循环节个数为n/L的置换有Euler(L)个
*/
/*
